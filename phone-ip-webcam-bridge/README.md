...
```bash
./phone-ip-webcam-bridge/bin/preflight.sh
```

2. Create local config:

```bash
cp ./phone-ip-webcam-bridge/config/bridge.example.env ./phone-ip-webcam-bridge/config/bridge.local.env
```
...
```bash
./phone-ip-webcam-bridge/bin/run-bridge.sh --config ./phone-ip-webcam-bridge/config/bridge.local.env
```
...
MAX_SECONDS=10 \
./phone-ip-webcam-bridge/bin/run-bridge.sh
```
...
```bash
./phone-ip-webcam-bridge/bin/run-bridge.sh --config ./phone-ip-webcam-bridge/config/bridge.local.env --dry-run
```
...
```bash
./phone-ip-webcam-bridge/bin/run-simulated-source.sh --mode file --duration 5 --output /tmp/phone-ip-webcam-sim.mp4
```

Optional RTSP publish attempt (environment-dependent ffmpeg behavior):

```bash
./phone-ip-webcam-bridge/bin/run-simulated-source.sh --mode rtsp --host 127.0.0.1 --port 8554 --path /live
```
...
./phone-ip-webcam-bridge/bin/run-bridge.sh --dry-run
```
...
```bash
./phone-ip-webcam-bridge/tests/emulation/run_docker_emulation.sh
```
