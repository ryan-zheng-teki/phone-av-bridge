...
select_python_cmd() {
  local candidates=()
  if [[ -n "${VOICE_CODEX_PYTHON:-}" ]]; then
    candidates+=("${VOICE_CODEX_PYTHON}")
  fi
...
if [[ ! -x "${VENV_PYTHON}" ]]; then
  if ! PYTHON_CMD="$(select_python_cmd)"; then
    echo "[install] no supported Python found. Install Python 3.${MIN_PY_MINOR}-3.${MAX_PY_MINOR} or set VOICE_CODEX_PYTHON." >&2
    exit 1
  fi
...
echo "[install] defaults: STT_MODEL=tiny.en, STT_LANGUAGE=en (LANG_CODE supported via .voice-codex.env)"
