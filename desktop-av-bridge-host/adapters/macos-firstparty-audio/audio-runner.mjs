...
export class MacOsFirstPartyAudioRunner {
  constructor({ enableSpeaker = false, streamUrl = '', outputDeviceName = 'PhoneAVBridgeAudio 2ch' } = {}) {
  ...
  async startMicrophoneRoute() {
    ...
    const phoneAvBridgeDriverInstalled = await driverBundleExists();
    ...
    if (!target) {
      ...
      if (!phoneAvBridgeDriverInstalled) {
        throw new Error(
          `${baseMessage}. Install first-party driver with: desktop-av-bridge-host/macos-audio-driver/scripts/install-driver.sh`
        );
      }
      throw new Error(`${baseMessage}. Reinstall PhoneAVBridgeAudio.driver and restart coreaudiod.`);
    }
