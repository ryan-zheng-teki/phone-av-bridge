...
export class MacOsCameraExtensionRunner {
  ...
  getCameraDeviceName() {
    return buildDeviceNames(this.deviceName, {
      cameraTarget: 'Phone AV Bridge Camera',
    }).camera;
  }
  ...
  async #ensureFrameServer() {
    ...
    throw new Error(
      `Phone AV Bridge Camera frame server is not reachable at tcp://${this.frameHost}:${this.framePort}. ` +
      'Open Phone AV Bridge Camera and approve Camera Extension in System Settings.'
    );
  }
  ...
  async #launchHostApp() {
    const candidates = [
      ...
      '/Applications/PhoneAVBridgeCamera.app',
      `${process.env.HOME || ''}/Applications/PhoneAVBridgeCamera.app`,
    ].filter(Boolean);
