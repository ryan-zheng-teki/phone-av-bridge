...
test('parseFfmpegAvfoundationDeviceList extracts audio and video sections', () => {
  const stderr = [
    ...
    '[AVFoundation indev @ 0x123] [1] PRC Camera',
    '[AVFoundation indev @ 0x123] AVFoundation audio devices:',
    '[AVFoundation indev @ 0x123] [0] MacBook Pro Microphone',
    '[AVFoundation indev @ 0x123] [1] PRCAudio 2ch',
  ].join('\n');
  ...
  assert.equal(result.audio[1].name, 'PRCAudio 2ch');
...
test('parseFfmpegAvfoundationDeviceList ignores lines outside sections', () => {
  const stderr = [
    ...
    '[AVFoundation indev @ 0x123] AVFoundation audio devices:',
    '[AVFoundation indev @ 0x123] [2] PRC Audio',
    'other text',
  ].join('\n');
  ...
  assert.equal(result.audio[0].name, 'PRC Audio');
});
