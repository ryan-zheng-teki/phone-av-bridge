...
OUTPUT_DEVICE="${MACOS_AUDIO_OUTPUT_DEVICE:-PRCAudio 2ch}"
...
cleanup() {
  ...
  docker rm -f prc-mediamtx >/dev/null 2>&1 || true
}
trap cleanup EXIT

cd "${HOST_DIR}"

pkill -f "node linux-app/server.mjs" >/dev/null 2>&1 || true
docker rm -f prc-mediamtx >/dev/null 2>&1 || true
docker run -d --name prc-mediamtx -p 8554:8554 bluenviron/mediamtx:latest >/dev/null
sleep 1

ffmpeg -hide_banner -loglevel warning -re \
  -f lavfi -i sine=frequency=1000:sample_rate=48000 \
  -c:a aac -b:a 128k \
  -f rtsp -rtsp_transport tcp "${RTSP_URL}" >/tmp/prc_rtsp_publish.log 2>&1 &
PUB_PID=$!
...
node linux-app/server.mjs >/tmp/prc_host.log 2>&1 &
HOST_PID=$!
sleep 2
...
sleep 2
curl -fsS "${HOST_BASE}/api/status" > /tmp/prc_status.json
mic="$(jq -r '.status.resources.microphone' /tmp/prc_status.json)"
spk="$(jq -r '.status.resources.speaker' /tmp/prc_status.json)"
issues="$(jq '.status.issues | length' /tmp/prc_status.json)"

if [[ "${mic}" != "true" || "${spk}" != "true" || "${issues}" != "0" ]]; then
  echo "macos-e2e failed: status not healthy" >&2
  jq '.status' /tmp/prc_status.json >&2
  exit 1
fi

speaker_raw="$(mktemp /tmp/prc_speaker_raw_XXXXXX)"
set +e
