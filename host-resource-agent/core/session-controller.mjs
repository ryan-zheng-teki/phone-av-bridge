...
function normalizeIssueMessage(resource, rawMessage) {
  ...
  if (resource === 'camera') {
    if (normalized.includes('camera extension') || normalized.includes('frame server')) {
      return 'Camera bridge is unavailable. Open PRCCamera and ensure Camera Extension approval is complete.';
    }
  ...
  if (resource === 'microphone') {
    if (normalized.includes('not found') && normalized.includes('audio')) {
      return 'Microphone route target is missing. Verify PRCAudio (macOS) or virtual mic sink (Linux).';
    }
