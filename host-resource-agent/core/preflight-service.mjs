...
import { probePrcAudioDriver } from '../adapters/macos-firstparty-audio/driver-probe.mjs';
...
export async function runPreflight(platform = process.platform) {
  ...
  if (platform === 'linux') {
    ...
    const prcExtensionState = await extensionStatus('org.autobyteus.prc.camera.extension');
    checks.push(
      checkResult(
        'macos_camera_extension_state',
        'PRC camera extension state',
        prcExtensionState === 'enabled' ? 'pass' : 'warn',
        prcExtensionState === 'enabled'
          ? 'PRC camera extension is activated and enabled.'
          : prcExtensionState === 'waiting_for_user'
            ? 'PRC camera extension is activated but waiting for user approval.'
            : prcExtensionState === 'missing'
              ? 'PRC camera extension is not installed.'
              : 'PRC camera extension status could not be confirmed.',
        prcExtensionState === 'enabled'
          ? null
          : 'Install/open PRCCamera.app and approve it under System Settings -> General -> Login Items & Extensions -> Camera Extensions.'
      )
    );

    const videoDevices = await listAvfoundationVideoDevices();
    const prcCameraVisible = videoDevices.some((device) =>
      device.name.toLowerCase().includes('phone resource companion camera')
    );
    checks.push(
      checkResult(
        'macos_virtual_camera_visible',
        'PRC virtual camera visible to capture APIs',
        prcCameraVisible ? 'pass' : 'warn',
        prcCameraVisible
          ? 'Phone Resource Companion Camera appears in AVFoundation video device list.'
          : 'Phone Resource Companion Camera is not visible in AVFoundation video device list.',
        prcCameraVisible
          ? null
          : 'Open PRCCamera.app, keep it running, then restart capture apps and rerun preflight.'
      )
    );
    ...
    checks.push(
      checkResult(
        ...
        'PRCCamera frame server',
        frameServerReady ? 'pass' : 'warn',
        frameServerReady
          ? 'PRCCamera frame server is listening on local TCP port 39501.'
          : 'PRCCamera frame server is not reachable on local TCP port 39501.',
        frameServerReady
          ? null
          : 'Open PRCCamera.app and verify extension approval before enabling camera on Android.'
      )
    );

    const audioDriverProbe = await probePrcAudioDriver({
      expectedDeviceName: process.env.MACOS_AUDIO_OUTPUT_DEVICE || 'PRCAudio 2ch',
    });
    checks.push(
      checkResult(
        'macos_prc_audio_driver_bundle',
        'PRCAudio driver bundle installed',
        audioDriverProbe.bundleExists ? 'pass' : 'warn',
        audioDriverProbe.bundleExists
          ? 'PRCAudio.driver is installed under /Library/Audio/Plug-Ins/HAL.'
          : 'PRCAudio.driver bundle not found.',
        audioDriverProbe.bundleExists
          ? null
          : 'Run: host-resource-agent/macos-audio-driver/scripts/install-driver.sh'
      )
    );

    checks.push(
      checkResult(
        'macos_prc_audio_device_visible',
        'PRCAudio virtual audio device visible',
        audioDriverProbe.visible ? 'pass' : 'warn',
        audioDriverProbe.visible
          ? `Virtual audio device "${process.env.MACOS_AUDIO_OUTPUT_DEVICE || 'PRCAudio 2ch'}" appears in capture APIs.`
          : `Virtual audio device "${process.env.MACOS_AUDIO_OUTPUT_DEVICE || 'PRCAudio 2ch'}" not visible in capture APIs.`,
        audioDriverProbe.visible
          ? null
          : 'Restart coreaudiod and reopen meeting/capture apps after installing PRCAudio.driver.'
      )
    );
    ...
    checks.push(
      checkResult(
        ...
        'This build uses PRCCamera extension + first-party PRCAudio driver for macOS media routing.',
        audioDriverProbe.bundleExists && audioDriverProbe.visible
          ? null
          : 'Install PRCAudio.driver and verify visibility before enabling microphone/speaker toggles.'
      )
    );
  } else {
    checks.push(
